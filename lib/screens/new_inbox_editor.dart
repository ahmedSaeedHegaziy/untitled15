...
import 'package:untitled15/models_localhost/activity.dart';
import 'package:untitled15/models_localhost/api_response.dart';
import 'package:untitled15/models_localhost/category.dart';
import 'package:untitled15/models_localhost/sender.dart';
import 'package:untitled15/models_localhost/status.dart';
import 'package:untitled15/models_localhost/user.dart';
import 'package:untitled15/screens/category_selector.dart';
import 'package:untitled15/screens/status_selector.dart';
import 'package:untitled15/services/sender_service.dart';
import 'package:untitled15/widgets/myAppBar.dart';
...
class NewInboxEditor extends StatefulWidget {
  final Function()? cancel;
  final Function()? done;

  const NewInboxEditor({Key? key, this.cancel, this.done}) : super(key: key);

  @override
  State<NewInboxEditor> createState() => _NewInboxState();
}

class _NewInboxState extends State<NewInboxEditor> {
  ...
  Future<void> getFile() async {
    try {
      final List<XFile>? pickedFileList = (await FilePicker.platform.pickFiles(
        allowMultiple: true,
      )) as List<XFile>?;

      if (pickedFileList != null) {
        setState(() {
          _imageFileList = pickedFileList.map((xfile) => File(xfile.path))
              .cast<XFile>()
              .toList();
        });
      }
  ...
  Widget build(BuildContext context) {
    final container = ProviderContainer(); // Create a new ProviderContainer
    final userRoleFuture = container.read(userRoleStateFuture);
      ...
      child: ClipRRect(
        ...
        child: Scaffold(
          ...
          body: _loading
            ...
            //                             '${status.name}'.tr(),
            //                             style: TextStyle(
            //                               color: Color(int.parse(status.color!)) == kYellow
            //                                   ? kBlack
            //                                   : kWhite,
            //                               fontWeight: FontWeight.w500,
            //                             ),
